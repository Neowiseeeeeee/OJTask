...
function MarqueeStrip({ reverse = false }: { reverse?: boolean }) {
  ...
  return (
    <div className="overflow-hidden py-3 select-none">
      <div
        className={`flex gap-0 w-max ${reverse ? "animate-marquee-reverse" : "animate-marquee"}`}
        style={{ willChange: "transform" }}
      >
        {doubled.map((item, i) => (
          <span
            key={i}
            className="inline-flex items-center gap-4 px-6 text-lg font-semibold text-white/80 whitespace-nowrap"
          >
...
// ── Feature cards ──────────────────────────────────────────────────────────
const features = [
  { icon: Clock, title: "Time Tracking", desc: "Log OJT hours daily and get instant supervisor sign-off. Zero spreadsheets.", color: "bg-violet-100 text-violet-600" },
  { icon: Users, title: "Daily Scrum", desc: "Two-minute structured check-ins that replace long status meetings for good.", color: "bg-indigo-100 text-indigo-600" },
  { icon: ListTodo, title: "Task Board", desc: "Kanban-style task management built around how interns actually work.", color: "bg-emerald-100 text-emerald-600" },
  { icon: CalendarDays, title: "Attendance", desc: "Real-time visibility for supervisors and coordinators with one tap.", color: "bg-amber-100 text-amber-600" },
  { icon: FileText, title: "Document Hub", desc: "Upload MOAs, endorsement letters, and reports in one secure place.", color: "bg-rose-100 text-rose-600" },
  { icon: MessageSquare, title: "Team Chat", desc: "Dedicated channels for work updates without flooding personal inboxes.", color: "bg-cyan-100 text-cyan-600" },
];
...
export default function LandingPage() {
  ...
  const [statsValues, setStatsValues] = useState({
    students: "...", companies: "...", schools: "...", satisfaction: "...",
  });
    ...
    <div className="min-h-screen bg-white text-slate-900 overflow-x-hidden">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center gap-4">
          <Link href="/" className="flex items-center gap-2.5 font-black text-xl tracking-tight text-slate-900 shrink-0">
          ...
          <div className="hidden md:flex items-center gap-2">
            <Link href="/auth">
              <Button variant="ghost" size="sm" className="font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-100" data-testid="link-nav-signin">
                Sign In
              </Button>
            </Link>
            <Link href="/auth">
        ...
        {menuOpen && (
          <div className="md:hidden bg-white border-t border-slate-100 px-6 py-4 flex gap-3">
      ...
      {/* ── HERO ─────────────────────────────────────────────── */}
      <section
        className="relative pt-36 pb-32 px-6 text-center overflow-hidden"
        style={{ background: "linear-gradient(160deg, #f5f3ff 0%, #ede9fe 40%, #ddd6fe 100%)" }}
      >
        {/* Soft blob decorations */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[900px] h-[500px] rounded-full bg-violet-300/20 blur-[100px] pointer-events-none" />
        <div className="absolute bottom-0 right-0 w-[400px] h-[400px] rounded-full bg-indigo-400/15 blur-[80px] pointer-events-none" />

        <div className="relative max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/70 backdrop-blur-sm border border-violet-200 text-violet-700 text-sm font-semibold mb-8 shadow-sm">
            <span className="w-2 h-2 rounded-full bg-violet-500 animate-pulse" />
            Built for OJT in the Philippines 🇵🇭
          </div>

          <h1 className="text-5xl md:text-7xl font-black tracking-tight leading-[1.04] mb-7 text-slate-900">
            Your Internship,{" "}
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
            <Link href="/auth">
              <Button
                size="lg"
                className="font-black text-base px-8 h-14 rounded-full bg-violet-600 hover:bg-violet-700 text-white shadow-xl shadow-violet-500/30 hover:shadow-violet-500/50 hover:-translate-y-0.5 transition-all"
            ...
            <Link href="/auth">
              <Button
                size="lg"
                variant="outline"
                className="font-bold text-base px-8 h-14 rounded-full border-2 border-slate-300 text-slate-700 hover:bg-white hover:border-slate-400 bg-white/70 backdrop-blur-sm"
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            <div key={s.label} data-testid={`stat-${s.label.toLowerCase().replace(/\s+/g, '-')}`}>
              <div className="text-4xl md:text-5xl font-black text-violet-600 mb-2">{s.value}</div>
      ...
      <section
        ...
        style={{ background: "linear-gradient(175deg, #fafafa 0%, #f5f3ff 100%)" }}
      >
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-violet-600 font-bold text-sm uppercase tracking-widest mb-4">Who Uses OJTask</p>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-5 text-slate-900">
              One Platform Built for{" "}
              <span className="text-violet-600">Everyone Involved</span>
            </h2>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {roles.map((role) => (
              <div
                key={role.title}
                className={`rounded-3xl ${role.light} ring-1 ${role.ring} p-8 hover:shadow-lg hover:-translate-y-1 transition-all duration-300`}
              ...
              >
                ...
                <h3 className="text-xl font-black mb-2 text-slate-900">{role.title}</h3>
                <p className="text-slate-500 text-sm mb-6 leading-relaxed">{role.subtitle}</p>
      ...
      <section id="features" className="py-28 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-violet-600 font-bold text-sm uppercase tracking-widest mb-4">Six Core Modules</p>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-5 text-slate-900">
              Everything Your OJT{" "}
              <span className="text-violet-600">Program Needs</span>
            </h2>
            <p className="text-slate-500 text-xl max-w-xl mx-auto leading-relaxed">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f) => (
              <div
                key={f.title}
                className="group p-7 rounded-3xl border border-slate-100 bg-white hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
              ...
              >
                ...
                <h3 className="text-lg font-black mb-2 text-slate-900">{f.title}</h3>
      ...
      <section
        id="how-it-works"
        className="py-28 px-6"
        style={{ background: "linear-gradient(175deg, #f5f3ff 0%, #ede9fe 100%)" }}
      >
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <p className="text-violet-600 font-bold text-sm uppercase tracking-widest mb-4">Getting Started</p>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-5 text-slate-900">
              Up and Running in{" "}
              <span className="text-violet-600">Five Minutes</span>
            </h2>
            <p className="text-slate-500 text-xl max-w-xl mx-auto leading-relaxed">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            {steps.map((step, i) => (
              <div
                key={step.step}
                className="relative group bg-white rounded-3xl p-7 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
                data-testid={`card-step-${step.step}`}
              >
                <div className="text-6xl font-black text-violet-100 absolute top-5 right-6 leading-none select-none">{step.step}</div>
      ...
      <section className="py-28 px-6 bg-white">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-6 text-slate-900">
            ...
            <span className="text-violet-600">Better Than Group Chats</span>
          </h2>
          ...
          <Link href="/auth">
            <Button
              size="lg"
              className="font-black text-base px-10 h-14 rounded-full bg-violet-600 hover:bg-violet-700 text-white shadow-xl shadow-violet-500/30 hover:shadow-violet-500/50 hover:-translate-y-0.5 transition-all"
              data-testid="button-cta-getstarted"
            >
              Get Started — it's free
            </Button>
          </Link>
          <p className="mt-5 text-sm text-slate-400">No credit card needed · Free to start</p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-10">
            ...
            <div className="col-span-2 md:col-span-1">
              ...
              <p className="text-white/60 text-sm leading-relaxed">
                The OJT management platform for Philippine schools and companies.
              </p>
            </div>
            ...
            {[
              {
                title: "Product",
                links: [
                  { label: "Time Tracking", href: "/auth" },
                  { label: "Daily Scrum", href: "/auth" },
                  { label: "Task Board", href: "/auth" },
                  { label: "Documents", href: "/auth" },
                ],
              },
              {
                title: "For",
                links: [
                  { label: "Students", href: "/auth" },
                  { label: "Supervisors", href: "/auth" },
                  { label: "Coordinators", href: "/auth" },
                  { label: "Schools", href: "/auth" },
                ],
              },
              {
                title: "Account",
              ...
              <div key={col.title}>
                ...
                <ul className="space-y-3">
                  {col.links.map((link) => (
                    <li key={link.label}>
                      <Link href={link.href} className="text-sm text-white/80 hover:text-white transition-colors font-medium">
                        {link.label}
                      </Link>
                    </li>
                  ))}
