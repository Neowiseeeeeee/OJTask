...
export default function MessagesPageFixed() {
  const { user } = useAuth();
  ...
  // Message component with proper alignment
  const MessageItem = ({ msg, isPreviousFromSameSender }: { 
    msg: any; 
    isPreviousFromSameSender: boolean;
  }) => {
    // More robust check for current user's message
    const currentUserId = user?.id;
    const messageSenderId = msg.senderId;
    const isMe = currentUserId && messageSenderId && currentUserId === messageSenderId;
    
    const senderId = msg?.senderId;
    const m = members.find((mm: any) => mm.userId === senderId);
    const displayName = isMe
      ? "You"
      : msg?.sender?.name ?? m?.user?.name ?? `User #${senderId}`;
    const profilePicture = isMe
      ? user?.profilePicture
      : msg?.sender?.profilePicture ?? m?.user?.profilePicture ?? null;
                ...
                [...messages]
                  ...
                  .map((msg, i, sorted) => {
                    const isPreviousFromSameSender =
                      i > 0 && sorted[i - 1].senderId === msg.senderId;
